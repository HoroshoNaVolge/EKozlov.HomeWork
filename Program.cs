...
TaskManager taskManager = new TaskManager(); // инициализируем экземпляр менеджера задач.
...
while (taskManager.AskToStart(isFirstExecution)) // Работа программы допускается после завершения конкретной задачи, возвращается в метод AskToStart (запуск выбора задач).
{
    ...
    ShowMessage(taskManager.CurrentTask.Result); //передаём в метод ShowMessage (в данной реализации вывод на консоль) string-свойство Result, обновленное по итогу выполнения метода Execute.
    ...
    // В методе AskToRepeat при выборе повтора задачи создается новый экземпляр конкретной задачи Task и присваевается свойству CurrentTask класса TaskManager.
    {
        ...
        ShowMessage(taskManager.CurrentTask.Result); // выводим результат повторного выполнения задачи.
    }
}